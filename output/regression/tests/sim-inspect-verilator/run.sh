...
(cd rustdv && cargo build --release -p framework_tests --quiet)

LIB="$TARGET_ROOT/release/libframework_tests.so"
[ -f "$LIB" ] || LIB="$TARGET_ROOT/release/libframework_tests.dylib"
RUSTDV_TESTCASE=inspect_visibility_ \
RUSTDV_VERILATOR_MODE=inspect \
RUSTDV_VERIFY_INSPECT_VISIBILITY=1 \
RUSTDV_VERILATOR_CONTROL_FILE="$PWD/sim/verilator-debug.vlt" \
    bash sim/run_verilator.sh "$LIB" tinyalu "$BUILD/verilator" \
